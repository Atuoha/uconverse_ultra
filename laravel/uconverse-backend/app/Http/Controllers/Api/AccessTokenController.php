...
class AccessTokenController extends Controller{
    ...
    public function get_rtc_token(Request $request){
       ...
       $appId = "cc52253064ae45c1bae33e1015348e7e";
       $appCertificate = "e8d90286fa764db1b328b9a7423d0123";
       $expireTimeInSeconds = 7200;
       $uid = 0;
       $access_token = new AccessToken2($appId, $appCertificate,$expireTimeInSeconds);
       $serviceRtc = new ServiceRtc($channelName, $uid);
       $serviceRtc->addPrivilege($serviceRtc::PRIVILEGE_JOIN_CHANNEL,$expireTimeInSeconds);
       $access_token->addService($serviceRtc);
       $token = $access_token->build();

       if(!empty($token)){
            return ['code'=>0,'data'=>$token,'msg'=>'success'];
       }
        return ['code'=>1,'data'=>"",'msg'=>'error occurred! Token error'];
